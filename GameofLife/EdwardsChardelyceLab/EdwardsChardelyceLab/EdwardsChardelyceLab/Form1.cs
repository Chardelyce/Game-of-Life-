...
#region hud 
                    ...
                    {
                        // DRAW HUD HERE
                         e.Graphics.DrawString(("GENERATIONS  = ") + generations.ToString() + (" Cells alive = ") + cellsalive.ToString() + (" Universe size =") + universe.GetLength(1) + universe.GetLength(0), thefonze, Brushes.White, huddy);
        ...
        private void otherSettingsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (select. ShowDialog() ==DialogResult.OK)
            {
                ...
                deadColor = select.b;
                cellColor = select.c;
                timer.Interval = select.peterpan = 20;
                //hook,lostboys
